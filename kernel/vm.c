...
kvmmake(void)
{
  ...
  kvmmap(kpgtbl, VIRTIO1, VIRTIO1, PGSIZE, PTE_R | PTE_W, 0);

  // PLIC
  kvmmap(kpgtbl, PLIC, PLIC, 0x400000, PTE_R | PTE_W, 0);
...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  if((*pte & PTE_V) == 0) {
      if(*pte & PTE_DS) {
          void *freePage = kalloc();
          if(!freePage) return 0;
          returnPage(pte, freePage);
          addPageToList(pagetable, va);
      } else return 0;
  }
  if((*pte & PTE_U) == 0)
    return 0;
...
void
kvmmap(pagetable_t kpgtbl, uint64 va, uint64 pa, uint64 sz, int perm, int canSwap)
{
  if(mappages(kpgtbl, va, sz, pa, perm, canSwap) != 0)
    panic("kvmmap");
}
...
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm, int canSwap)
{
  ...
  for(;;){
    ...
    if(canSwap) addPageToList(pagetable, a);
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    ...
    removePageFromList(pagetable, a);
    *pte = 0;
  }
}
